        ...
        /* End of statement */
            ...
            if (esp_gps->crc == crc) {
                ...
                if (((esp_gps->parsed_statement) & esp_gps->all_statements) == esp_gps->all_statements) {
                  ...
                  //          " altitude = %.02fm"
                  //          " speed = %fm/s",
                  //          k++, (esp_gps->parent).date.year + 2000, (esp_gps->parent).date.month, (esp_gps->parent).date.day,
                  //          (esp_gps->parent).tim.hour + 7, (esp_gps->parent).tim.minute, (esp_gps->parent).tim.second,
                  //          (esp_gps->parent).latitude, (esp_gps->parent).longitude, (esp_gps->parent).altitude, (esp_gps->parent).speed);

                           gps_info = (esp_gps->parent);
                }
            } else {
